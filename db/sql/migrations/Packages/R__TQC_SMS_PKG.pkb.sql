--
-- TQC_SMS_PKG  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_sms_pkg
AS
...
--v_reciepient'||v_reciepient||'v_clnt_code'||v_clnt_code||'v_sms_tel'||v_sms_tel);
      IF v_reciepient IN ('C', 'R')
      THEN
         BEGIN
            SELECT clnt_sms_tel
              INTO v_sms_tel
...
--       raise_error ('Error client not defined..'||'
--v_reciepient'||v_reciepient||'v_clnt_code'||v_clnt_code||'v_sms_tel'||v_sms_tel);
--       ELSIF v_reciepient = 'R'
--      THEN
----      raise_error ('v_reciepient'||v_reciepient||'v_clnt_code'||v_clnt_code||'v_sms_tel'||v_sms_tel);
--         BEGIN
--            SELECT clnt_sms_tel
--              INTO v_sms_tel
--              FROM tqc_clients
...
--     raise_error('v_usr_code'||v_usr_code||'v_usr_type'||v_usr_type);
         IF v_usr_type = 'U'
         THEN
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error ('Template selected not found..' || v_msgt_code);
      END;

      BEGIN
--       raise_error(v_pol_code||'==='||
--                                 v_clm_no||'==='||
--                                 v_sms_msg||'==='||
--                                 v_app_lvl||'==='||
...
--                                 v_trans_no );
         v_sms_msg :=
            tq_crm.tqc_memos_dbpkg.process_gis_pol_memo (
--                                  tq_crm.process_gis_pol_memo_2(
                                                         v_pol_code,
                                                         v_clm_no,
                                                         v_sms_msg,
                                                         v_app_lvl,
...
--      raise_error('v_sms_msg'||v_sms_msg);
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error ('Unable to merge sms memo! ' || SQLERRM);
      END;
...
--         );
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error ('Error Sending sms message!' || SQLERRM);
      END;
   END;
   ...
   LOOP
    begin
      vposttext :=
         ...
         || CHR (38)
         || 'MSISDN='
         || utl_url.ESCAPE (REPLACE(cur_sms_rec.sms_tel_no, '+', ''), TRUE)
         || CHR (38)
         || 'MESSAGE='
...
--       raise_error('v_url='||v_url||'v_username='||'v_password ='||v_password||'v_source='||v_source);  
      BEGIN
      --utl_http.set_proxy('http://heritage\Turnkey.Support:Password1@10.235.140.10:8080');
...
PROCEDURE create_telematic_sms_msg (
   ...
   BEGIN
      ...
      THEN
         ...
         IF v_usr_type = 'U'
         THEN
            SELECT REPLACE(usr_cell_phone_no, '+', '')
              INTO v_sms_tel
              FROM tqc_users
             WHERE usr_code = v_usr_code;
         ELSE                                            -- HANDLE GROUP USERS
