--
-- TQC_MEMO_WEB_PKG  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.TQC_MEMO_WEB_PKG AS
  ...
  BEGIN
    ...
    IF v_tran_type = 'N' THEN
      BEGIN
        ...
        --        TQC_MEMO_TYPES
        SELECT TO_NUMBER(TO_CHAR(SYSDATE, 'YYYY'))|| TQC_COMEM_CODE_SEQ.NEXTVAL INTO v_mem_code FROM DUAL;
     
        v_refno := nvl(v_usr_ref, v_user) || '/' || v_mem_code;
      --RAISE_ERROR ('TEDD WAS HERE'||v_mem_code||';'||v_refno);
        INSERT INTO TQC_COMPANY_MEMOS
          (COMEM_CODE,
           COMEM_SYS_CODE,
           COMEM_DESC,
        ...
        UPDATE TQC_COMPANY_MEMOS
           SET COMEM_DESC             = NVL(v_mem_desc, COMEM_DESC),
              ...
              -- COMEM_MTYP_CODE        = NVL(v_mtyp_code, COMEM_MTYP_CODE),
               ...
               comem_prp_code         = v_prp_code
         WHERE COMEM_CODE = v_mem_pk;
        ...
        IF NVL(v_mem_count, 0) > 0 THEN
         ...
         -- RAISE_ERROR('v_mem_code'||v_mem_code);
            SELECT MAX(MEMH_CODE)
              INTO v_memh_code
              FROM TQC_MEMO_HEADER
             WHERE MEMH_COMEM_CODE = v_mem_pk;
            ...
            WHEN NO_DATA_FOUND THEN
              ...
              INSERT INTO TQC_MEMO_HEADER
                (MEMH_CODE,
                 ...
                 MEMH_MEMO_SUBJECT)
              VALUES
                (v_memh_code, v_mem_pk, v_memo_code, v_memo_subject);
...
PROCEDURE authorise_memo(v_user      VARCHAR2,
                           ...
                           v_sign      OUT VARCHAR2,
                            v_seq_type   VARCHAR2,
                            v_ref_no OUT VARCHAR2) IS
    v_name VARCHAR2(30);
    v_rank VARCHAR2(60);
    ncode  number;
    v_seq_nos  VARCHAR2(100);
    v_pro_code number;
    v_brn_code  number;
    v_uwyr   number;
  BEGIN
   --RAISE_ERROR('v_memo_code '||v_memo_code||'v_seq_type=='||v_seq_type||'v_memo_code=='||v_memo_code);
    BEGIN
      SELECT USR_NAME,
             decode(USR_SIGN,
                    'Y',
    ...
    BEGIN
 --the use of sequences with types defined from CRM
    IF NVL(v_seq_type,'L') ='L'  THEN
    --if not defined will work as was accustomed to
      UPDATE TQC_COMPANY_MEMOS
      ...
      ELSE  
      ---if seq type defined
SELECT POL_PRO_CODE, POL_BRN_CODE, POL_UW_YEAR
  INTO v_pro_code, v_brn_code, v_uwyr
  FROM GIN_POLICIES
 WHERE POL_BATCH_NO IN (SELECT COMEM_POL_BATCH_NO
                          FROM TQC_COMPANY_MEMOS
                         WHERE COMEM_CODE = v_memo_code);
      
      ---get the sequence  no
      
      --raise_error('v_seq_type=='||v_seq_type||'v_pro_code=='||v_pro_code||'v_brn_code=='||v_brn_code||'v_uwyr=='||v_uwyr);
       v_seq_nos :=
...
--             
      ...
      --  RAISE_ERROR('vcoverfrom'||vcoverfrom);
          lv_temp_txt := REPLACE(lv_temp_txt, '[POLICYNO]', v_pol_no);
          lv_temp_txt := REPLACE(lv_temp_txt, '[POLNO]', v_pol_no);
          lv_temp_txt := REPLACE(lv_temp_txt,
                                 '[COVERFROM]',
          ...
          lv_temp_txt := REPLACE(lv_temp_txt,
               ...
               --RAISE_ERROR('lv_temp_txt '||lv_temp_txt);                      
--        EXCEPTION
--          WHEN NO_DATA_FOUND THEN
--            NULL;
--          WHEN OTHERS THEN
      ...
      BEGIN
        ...
        BEGIN  
          SELECT DISTINCT spr_name
            ...
            AND  APCO_CMB_CLAIM_NO=v_claim_no
            AND ROWNUM=1;
          ...
          WHEN NO_DATA_FOUND THEN
            NULL;
          WHEN OTHERS THEN
            RAISE_ERROR('Unable to gET Service Provider Details ' || SQLERRM(SQLCODE));
...
END;
  FUNCTION MERGE_DOCS_RQRD(v_claim_no      IN VARCHAR2,
                         ...
                         v_doc_submited  IN VARCHAR2) RETURN VARCHAR2 IS

    Cursor rqrd_docs IS SELECT TO_CHAR(ROWNUM) ||' . '||RDOC_DESC RDOC_DESC,ROWNUM v_row
                        FROM GIN_CLM_DOC_REQRD_SUBMTD,GIN_REQRD_DOCUMENTS
                        WHERE DOCRC_RDOC_ID = RDOC_ID
                        AND DOCRC_CMB_CLAIM_NO = v_claim_no
                        AND DOCRC_SUBMITED = v_doc_submited
                        ORDER BY DOCRC_ID;
                        
    v_rqrd_docs VARCHAR2(4000);                    
    BEGIN
        FOR x IN rqrd_docs LOOP
            IF v_claim_no IS NOT NULL THEN
               v_rqrd_docs := v_rqrd_docs||'
               '||x.RDOC_DESC; 
            END IF;
        END LOOP;   
        RETURN(v_rqrd_docs);   
