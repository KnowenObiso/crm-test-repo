--
-- TQC_ROLES_CURSOR030215  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.TQC_ROLES_CURSOR030215
AS
   /******************************************************************************
   ...
   ******************************************************************************/
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT SRLS_CODE,
                ...
                SRLS_STATUS
           FROM TQC_SYS_ROLES
          WHERE SRLS_STATUS = 'A' AND SRLS_SYS_CODE = v_sys_code
                AND NOT EXISTS
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT SRLS_CODE,
                ...
                SRLS_STATUS
           FROM TQC_SYS_ROLES
          WHERE SRLS_STATUS NOT IN ('N', 'I') AND SRLS_SYS_CODE = v_sys_code
                AND EXISTS
                       (SELECT USRR_USR_CODE, USRR_SRLS_CODE
                          FROM TQC_SYS_USER_ROLES
                         WHERE TQC_SYS_ROLES.SRLS_CODE =
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT DIV_CODE,
          ...
          WHERE DIV_CODE = BDIV_DIV_CODE AND BDIV_BRN_CODE = v_branch_code
                AND NOT EXISTS
                           (SELECT USD_CODE,
                                   USD_USR_CODE,
                                   USD_DIV_CODE,
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT ORG_CODE, ORG_SHT_DESC, ORG_NAME FROM TQC_ORGANIZATIONS;
   ...
   BEGIN
      ...
      INSERT INTO TQC_USERS (USR_CODE,
                             USR_USERNAME,
                             USR_NAME,
                             USR_PWD,
                             USR_EMAIL,
           ...
           VALUES (v_user_code,
                   UPPER (v_username),
                   UPPER (v_names),
                   DECODE (v_type, 'U', GIS_READ.VAL (v_password), NULL),
                   v_email,
                   UPPER (v_personel),
                   UPPER (v_per_id),
                   UPPER (v_status),
                   ...
                   SYSDATE,
                   UPPER (v_reset),
                   UPPER (v_usr_acct_mgr),
                   v_usr_signature,
                   V_USR_CELL_PHONE_NO,
                   v_usr_wef_dt,
                   v_usr_wet_dt,
                   V_USR_SECQ_CODE,
                   UPPER (V_USR_SECURITY_ANSWER));
   END Create_User;
   ...
   BEGIN
      ...
      UPDATE TQC_USERS
         SET USR_USERNAME = UPPER (v_username),
             USR_NAME = UPPER (v_names),
             USR_EMAIL = NVL (v_email, USR_EMAIL),
             USR_PERSONEL_RANK = UPPER (NVL (v_personel, USR_PERSONEL_RANK)),
             USR_PER_ID = UPPER (NVL (v_per_id, USR_PER_ID)),
             USR_TYPE = UPPER (NVL (v_type, USR_TYPE)),
             USR_PWD_RESET = UPPER (NVL (v_reset, USR_PWD_RESET)),
             USR_STATUS = UPPER (NVL (v_status, USR_STATUS)),
             USR_LOGIN_ATTEMPTS = 0,
             USR_ACCT_MGR = UPPER (NVL (v_usr_acct_mgr, USR_ACCT_MGR)),
             USR_CELL_PHONE_NO = v_USR_CELL_PHONE_NO,
             USR_WEF_DT = v_USR_WEF_DT,
             USR_WET_DT = v_USR_WET_DT,
             USR_SECQ_CODE = V_USR_SECQ_CODE,
             USR_SECURITY_ANSWER = UPPER (V_USR_SECURITY_ANSWER)
   ...
   BEGIN
      ...
      IF (NVL (v_Count, 0) = 0)
      THEN
         INSERT INTO TQC_USER_SYSTEMS (USYS_CODE,
                                       USYS_USR_CODE,
                                       USYS_SYS_CODE,
                                       USYS_WEF,
                                       USYS_WET)
   ...
   BEGIN
      ...
      IF (NVL (v_count, 0) = 0)
      THEN
         INSERT INTO TQC_SYS_USER_ROLES (USRR_USR_CODE,
                                         USRR_SRLS_CODE,
                                         USRR_GRANT_DATE,
                                         USRR_REVOKE_DATE,
                                         USRR_GRANT_BY_USR_CODE,
      ...
      ELSE
         UPDATE TQC_SYS_USER_ROLES
            SET USRR_REVOKE_DATE = NULL, USRR_REVOKE_BY_USR_CODE = NULL
          WHERE USRR_USR_CODE = v_user AND USRR_SRLS_CODE = v_role;
      END IF;
   END Grant_User_Role;
   ...
   BEGIN
      ...
      LOOP
         ...
         EXCEPTION
            ...
            THEN
               RAISE_ERROR (
                  'Error determining is process has been granted..');
         END;
         ...
         LOOP
            DBMS_OUTPUT.PUT_LINE (
               subArea_rec.SPRSA_CODE || '=' || v_roleCode);
   ...
   BEGIN
      ...
      THEN
         ...
         EXCEPTION
            ...
            THEN
               RAISE_ERROR (
                     'Error granting process RoleCode is '
                  || v_roleCode
                  || ' :: sprcaCode is '
                  || v_sprca_code);
         END;
...
*/
   ...
   BEGIN
      ...
      THEN
         ...
         INSERT INTO TQC_USER_BRANCHES (USB_CODE,
                                        USB_BRN_CODE,
                                        USB_USR_CODE,
                                        USB_STATUS,
                                        USB_DFLT_BRN)
   ...
   BEGIN
      ...
      LOOP
         ...
         EXCEPTION
            ...
            THEN
               RAISE_ERROR (
                     'Error assigning process to the role..'
                  || v_processCode
                  || '='
                  || v_roleCode
                  || '='
   ...
   */
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT REG_CODE, REG_ORG_CODE, REG_NAME
           FROM TQC_REGIONS
          WHERE REG_ORG_CODE = v_orgCode
                AND REG_CODE IN (SELECT REG_CODE
                                   FROM (  SELECT COUNT (1),
                                                  REG_CODE,
                                                  REG_ORG_CODE
   ...
   IS
      ...
      IS
         SELECT SRLS_CODE,
                ...
                SRLS_STATUS
           FROM TQC_SYS_ROLES
          WHERE SRLS_STATUS <> 'N' AND SRLS_SYS_CODE = v_sys_code
                AND NOT EXISTS
                           (SELECT USRR_USR_CODE, USRR_SRLS_CODE
                              FROM TQC_SYS_USER_ROLES
                             WHERE TQC_SYS_ROLES.SRLS_CODE =
   ...
   IS
      ...
      IS
         SELECT SRLS_CODE,
                ...
                SRLS_STATUS
           FROM TQC_SYS_ROLES
          WHERE SRLS_STATUS <> 'I' AND SRLS_SYS_CODE = v_sys_code
                AND EXISTS
                       (SELECT USRR_USR_CODE, USRR_SRLS_CODE
                          FROM TQC_SYS_USER_ROLES
                         WHERE TQC_SYS_ROLES.SRLS_CODE =
                ...
                AND EXISTS
                       (SELECT USYS_CODE, USYS_USR_CODE, USYS_SYS_CODE
                         ...
                         WHERE TQC_SYSTEMS.SYS_CODE =
                                  TQC_USER_SYSTEMS.USYS_SYS_CODE
                               AND USYS_USR_CODE = v_user_code)
                AND SYS_CODE IN (SELECT SYS_CODE
                                   FROM (  SELECT SRLS_SYS_CODE AS SYS_CODE,
                                                  COUNT (*)
                                             FROM TQC_SYS_ROLES
   ...
   IS
      ...
      CURSOR USERS_CUR
      IS
         SELECT USR_USERNAME FROM TQC_USERS;
   ...
   BEGIN
      ...
      THEN
         ...
         EXCEPTION
            ...
            THEN
               RAISE_ERROR (
                  'ERROR CREATING THE ADMINISTRATOR ACCOUNT'
                  || SQLERRM (SQLCODE));
         END;
      ELSE
         BEGIN
   ...
   BEGIN
      UPDATE tqc_users
         SET usr_pwd = gis_read.val (v_password)
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT ACCC_CODE, 
           ...
           WHERE ACCC_AGN_CODE = v_agent_code OR v_agent_code = 0 --Allow DIRECT Agent to view all
          ORDER BY ACCC_USERNAME;
