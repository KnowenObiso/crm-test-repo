...
--
-- TQC_CLIENTS_PKG_06022017  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_clients_pkg_06022017
AS
/******************************************************************************
...
--    RAISE_ERROR('v_cltn_client_types'||v_cltn_client_types);
   --RAISE_ERROR('clnt_spz_code='||v_clnt_spz_code);
--   raise_error(
...
--         END IF;
      ...
      IF v_pin IS NOT NULL AND (v_direct_clnt!='I' or (v_direct_clnt='I' and v_age>=18))
      THEN
         BEGIN
            SELECT NVL (COUNT (*), 0)
              INTO v_id_cnt
     ...
     IF v_sms_tel IS NOT NULL
      THEN
         ...
         IF NVL(v_sms_tel_count,0) = 0 THEN
             BEGIN
            -- RAISE_ERROR('v_sms_tel='|| v_sms_tel);
