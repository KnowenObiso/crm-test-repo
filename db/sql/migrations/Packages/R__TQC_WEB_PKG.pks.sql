...
PROCEDURE getPersonnelRank (v_user_code IN NUMBER, v_personnel_rank OUT VARCHAR2);
   PROCEDURE get_sys_system_users(v_syscode       IN NUMBER,
                               v_div_code      IN NUMBER,
                               v_grp_user      IN  NUMBER,
                               v_sys_users_cur OUT sys_users_rec);
PROCEDURE request_client_password_reset (v_email IN VARCHAR2, v_email_token IN VARCHAR2, v_sms_token IN VARCHAR2);
...
PROCEDURE assign_ticket ( 
    ...
    --07022018-ABOUT TICKETING ISSUE
    TYPE current_tickets_rec IS RECORD (
      tckt_code              tqc_bpm_tickets.tckt_code%TYPE,
      sysmodule              tqc_bpm_tickets.tckt_sys_module%TYPE,
      tckt_entity_code       tqc_bpm_tickets.tckt_entity_code%TYPE,
      entity_ref_no          tqc_clients.clnt_sht_desc%TYPE,
      entity_name            tqc_clients.clnt_name%TYPE,
      tckt_by                tqc_bpm_tickets.tckt_by%TYPE,
      tckt_date              tqc_bpm_tickets.tckt_date%TYPE,
      tckt_process_id        tqc_bpm_tickets.tckt_process_id%TYPE,
      tckt_sys_module        tqc_bpm_tickets.tckt_sys_module%TYPE,
      tckt_to                tqc_bpm_tickets.tckt_to%TYPE,
      tckt_remarks           tqc_bpm_tickets.tckt_remarks%TYPE,
      name_                  jbpm4_task.name_%TYPE,
      duedate_               jbpm4_task.duedate_%TYPE,
      tckt_transno           tqc_bpm_tickets.tckt_transno%TYPE,
      tckt_type              tqc_bpm_tickets.tckt_type%TYPE,
      tckt_assignment_date   tqc_bpm_tickets.tckt_assignment_date%TYPE,
      scrn_name              tqc_bpm_screens.scrn_name%TYPE,
      sprc_jpdl_desc         tqc_sys_processes.sprc_jpdl_desc%TYPE,
      sprc_sht_desc          tqc_sys_processes.sprc_sht_desc%TYPE
   );

   TYPE current_tickets_ref IS REF CURSOR
      RETURN current_tickets_rec;
    --07022018
END tqc_web_pkg;
/