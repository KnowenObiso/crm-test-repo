--
-- TQC_PORTAL_CURSOR01112015  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM."TQC_PORTAL_CURSOR01112015" AS
/******************************************************************************
...
FUNCTION getProductCategories(v_sysCode NUMBER, v_prodCode IN NUMBER DEFAULT NULL, v_agentCode IN NUMBER DEFAULT NULL) RETURN product_categories_ref IS
 v_cursor product_categories_ref;
 BEGIN
-- RAISE_ERROR('v_prodCode: ' || v_prodCode || ' v_agentCode: ' || v_agentCode);
    IF v_prodCode IS NULL AND v_agentCode IS NULL THEN
     OPEN v_cursor FOR
         SELECT DISTINCT TWPC_CODE, TWPC_SYS_CODE, TWPC_NAME, TWPC_DESCRIPTION
         FROM TQC_WEB_PRODUCT_CATEGORIES, TQC_WEB_PRODUCTS
         WHERE TWPC_SYS_CODE = v_sysCode
         AND TWPC_CODE = TWP_TWPC_CODE
         ORDER BY TWPC_NAME;
    ELSIF v_agentCode IS NOT NULL THEN
            OPEN v_cursor FOR
             SELECT DISTINCT TWP_PROD_CODE, v_sysCode, PRO_DESC, TWP_PROD_DESC
             FROM TQC_WEB_PRODUCTS, GIN_PRODUCTS, TQC_AGENCY_PRODUCTS
...
-- RAISE_ERROR('v_prodCode'||v_prodCode||'v_sysCode'||v_sysCode);
 ...
 OPEN v_cursor FOR
 SELECT POL_BATCH_NO POL_CODE_BATCH_NO, 37 SYS_CODE, POL_POLICY_NO POLICY_NUMBER, POL_QUOT_NO PROP_QUOTE_NO, PRO_DESC PROD_DESC, 
 POL_POLICY_COVER_FROM COVER_FROM, POL_POLICY_COVER_TO COVER_TO,
 POL_TOTAL_SUM_INSURED AMOUNT_INSURED,POL_CURRENT_STATUS STATUS,POL_PREPARED_BY DONE_BY,
 POL_INCEPTION_DT POL_INCEPTION_DT,NULL QUOT_OK, NULL PROD_TYPE
 ...
 UNION
 SELECT QUOT_CODE POL_CODE_BATCH_NO, 37 SYS_CODE, NULL POLICY_NUMBER, QUOT_NO PROP_QUOTE_NO, PRO_DESC PROD_DESC,
 QP_WEF_DATE COVER_FROM, QP_WET_DATE COVER_TO,
 QP_TOTAL_SI AMOUNT_INSURED,QUOT_STATUS STATUS,QUOT_PREPARED_BY DONE_BY,
 QUOT_DATE POL_INCEPTION_DT,QUOT_LOAN_DISBURSED QUOT_OK, NULL PROD_TYPE
 ...
 UNION
 SELECT POL_CODE,27,POL_POLICY_NO,POL_PROPOSAL_NO,PROD_DESC,POL_EFFECTIVE_DATE, 
