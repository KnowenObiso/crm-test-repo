...
--
-- TQC_CLIENTS_PKG  (Package) 
--
CREATE OR REPLACE PACKAGE TQ_CRM.tqc_clients_pkg
AS
/******************************************************************************
...
******************************************************************************/
   ...
   TYPE client_accounts_rec IS RECORD (
      ...
      clna_cnt_name       tqc_client_accounts.clna_cnt_name%type,
      clna_cnt_title      tqc_client_accounts.clna_cnt_title%type,
      clna_cnt_smsno      tqc_client_accounts.clna_cnt_smsno%type,
      clna_cnt_email      tqc_client_accounts.clna_cnt_email%type
   );
   ...
   TYPE clients_dtls_rec IS RECORD (
      ...
      question1                 tqc_security_questions.secq_desc%TYPE,
      question2                 tqc_security_questions.secq_desc%TYPE
   );
  ...
  TYPE clients_details_rec IS RECORD (
        ...
        PNAME                     VARCHAR2 (300),
        CLNT_BRN_CODE             TQC_CLIENTS.CLNT_BRN_CODE%TYPE,
        CLNT_ACC_OFFICER          VARCHAR2(200),
        CLNT_ACC_NAME             VARCHAR2(200),
        CLNT_OLD_SHT_DESC         VARCHAR2(200),
        CLT_CELL_NO               VARCHAR2(200),
        BANKBRANCH_NAME           VARCHAR2(50),
        CLNT_OCCUPATION           TQC_CLIENTS.CLNT_OCCUPATION%TYPE,
        CLNT_ANNIVERSARY          TQC_CLIENTS.CLNT_ANNIVERSARY%TYPE ,
        CLNT_CRDT_RATING          TQC_CLIENTS.CLNT_CRDT_RATING%TYPE,
        CLTN_CLIENT_TYPES         TQC_CLIENTS.CLTN_CLIENT_TYPES%TYPE,
        HOLDING_CLNT                VARCHAR2(200),
        CLNT_SACCO                     TQC_CLIENTS.CLNT_SACCO%TYPE,
        COU_ZIP_CODE                VARCHAR2(200),
        CLNT_CREDIT_LIM_ALLOWED        TQC_CLIENTS.CLNT_CREDIT_LIM_ALLOWED%TYPE, 
        CLNT_CREDIT_LIMIT            TQC_CLIENTS.CLNT_CREDIT_LIMIT%TYPE,
        LOC_NAME                    VARCHAR2(200),
        CLNT_BOUNCED_CHQ            TQC_CLIENTS.CLNT_BOUNCED_CHQ%TYPE,
        CLNT_MARITAL_STATUS            TQC_CLIENTS.CLNT_MARITAL_STATUS%TYPE,
        CLNT_DEFAULT_COMM_MODE        TQC_CLIENTS.CLNT_DEFAULT_COMM_MODE%TYPE,
        CLNT_PAYROLL_NO                TQC_CLIENTS.CLNT_PAYROLL_NO%TYPE,
        CLNT_SAL_MIN_RANGE            TQC_CLIENTS.CLNT_SAL_MIN_RANGE%TYPE,
        CLNT_SAL_MAX_RANGE            TQC_CLIENTS.CLNT_SAL_MAX_RANGE%TYPE,
        CLNT_BPN_CODE                TQC_CLIENTS.CLNT_BPN_CODE%TYPE,
        BPN_NAME                    VARCHAR2(200),
        CLNT_TEL_PAY                TQC_CLIENTS.CLNT_TEL_PAY%TYPE,
        CLNT_INT_TEL                TQC_CLIENTS.CLNT_INT_TEL%TYPE, 
        CLNT_EMAIL2                 TQC_CLIENTS.CLNT_EMAIL2%TYPE,
        CLNT_WORK_PERMIT             TQC_CLIENTS.CLNT_WORK_PERMIT%TYPE,
        CLNT_DL_ISSUE_DATE             TQC_CLIENTS.CLNT_DL_ISSUE_DATE%TYPE,
        CLNT_SMS_PREFIX             TQC_CLIENTS.CLNT_SMS_PREFIX%TYPE,
        CLNT_COMM_MODE                 TQC_CLIENTS.CLNT_COMM_MODE%TYPE
   );
...
TYPE tqc_acccounts_agents_rec IS RECORD(
     ...
     v_agn_code             tqc_agencies.AGN_CODE%TYPE,
     v_act_type_sht_desc    tqc_account_types.ACT_TYPE_SHT_DESC%TYPE ,
     v_agn_bank_acc_no      tqc_agencies.agn_bank_acc_no%TYPE ,
 ...
 PROCEDURE get_tqc_accounts_agents(v_acc_type IN        NUMBER,
                                    v_cursor OUT        tqc_accounts_agents_cursor,
                                    v_agnt_id           VARCHAR2,
                                    v_agent             VARCHAR2);
...
FUNCTION create_client_proc(v_direct_clnt           IN VARCHAR2,
   ...
   TYPE getclienttypes_rec IS RECORD (
      ...
      clnty_clnt_type   tqc_client_types.clnty_clnt_type%TYPE,
      clnty_category    tqc_client_types.clnty_category%TYPE,
      clnty_person      tqc_client_types.clnty_person%TYPE
   );
   ...
   FUNCTION getPreferedCommMode(v_clnt_code IN NUMBER, commMode IN VARCHAR2) 
        RETURN VARCHAR2;
TYPE Client_name_rec IS RECORD
(
        PRP_CODE NUMBER,
...
TYPE Client_name_def IS REF CURSOR RETURN Client_name_rec; 
PROCEDURE Prpclients(v_client_name_rec OUT Client_name_def, v_agen_code NUMBER);
PROCEDURE get_pymt_account_types(v_cursor OUT tqc_account_type_cursor);    
FUNCTION getClientName (v_clientCode IN NUMBER)
RETURN VARCHAR2;

 TYPE   doc_types_rec IS RECORD(
        ed_sht_desc   tqc_ecm_docs.ed_sht_desc%type,
        ed_desc       tqc_ecm_docs.ed_desc%type
 );
 
 TYPE doc_types_ref IS REF CURSOR
 RETURN doc_types_rec;
...
TYPE client_employer_rec IS RECORD (
      code              tqc_client_employers.code%TYPE,
      clnt_code         tqc_client_employers.clnt_code%TYPE,
      cou_code          tqc_client_employers.cou_code%TYPE,
      cou_name          tqc_countries.cou_name%TYPE,
      state_code       tqc_client_employers.sector_code%type,
      state_name       tqc_states.sts_name%type,
      town_code         tqc_client_employers.town_code%TYPE,
      twn_name          tqc_towns.twn_name%TYPE,
      -- pst_code          tqc_client_employers.pst_code%TYPE,
      pst_zip_code      tqc_postal_codes.pst_zip_code%TYPE,
      sector_code       tqc_client_employers.sector_code%TYPE,
      sec_name          tqc_sectors.sec_name%TYPE,
      employer_type     tqc_client_employers.employer_type%TYPE,
      employer_name     tqc_client_employers.employer_name%TYPE,
      payroll_no        tqc_client_employers.payroll_no%TYPE,
      min_salary        tqc_client_employers.min_salary%TYPE,
      max_salary        tqc_client_employers.max_salary%TYPE,
      monthly_income    tqc_client_employers.monthly_income%TYPE,
      employment_date   tqc_client_employers.employment_date%TYPE,
      employer_nos      tqc_client_employers.employer_nos%TYPE,
      employer_cells    tqc_client_employers.employer_cells%TYPE,
      employer_fax      tqc_client_employers.employer_fax%TYPE
   );

   TYPE client_employer_ref IS REF CURSOR
      RETURN client_employer_rec;
...
FUNCTION get_client_by_token (v_token IN varchar2)
      ...
      TYPE tqc_client_type_rec IS RECORD (
      clnty_code        NUMBER,
      clnty_clnt_type   VARCHAR2 (200 BYTE),
      clnty_category    VARCHAR2 (40 BYTE),
      clnty_person      VARCHAR2 (1 BYTE),
      clnty_desc        VARCHAR2 (100 BYTE),
      clnty_type        VARCHAR2 (100 BYTE)
   );

   TYPE tqc_client_type_ref IS REF CURSOR
      RETURN tqc_client_type_rec;      
