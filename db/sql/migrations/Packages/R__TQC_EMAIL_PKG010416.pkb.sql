--
-- TQC_EMAIL_PKG010416  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_email_pkg010416
AS
...
--raise_Error(v_usr_type||'='||v_usr_code);
   ...
   PROCEDURE create_email_msg09102015 (
      v_reciepient     IN       VARCHAR2,
      v_clnt_code      IN       NUMBER,
      v_agn_code       IN       NUMBER,
      v_quot_code      IN       NUMBER,
...
-- for the "attach" parameters in the interfaces above).
   ...
   Function  getClaimEmailMessage(v_sys_code NUMBER DEFAULT NULL)
   RETURN VARCHAR2
   IS
   ...
   BEGIN
            SELECT  MSGT_MSG
                INTO    v_message
            FROM    TQC_MSG_TEMPLATES
            WHERE   MSGT_SHT_DESC = 'PRTL_EML_MSG'
                AND MSGT_SYS_CODE = NVL(v_sys_code, MSGT_SYS_CODE);
                RETURN(v_message);
        EXCEPTION WHEN OTHERS THEN
            v_message := 'Kindly find the attached required documents for the claim number [CLAIMNUMBER]. Note these documents will be required for the claim to be pocessed.';
         
        RETURN(v_message);
   END getClaimEmailMessage;
END tqc_email_pkg010416; 
/