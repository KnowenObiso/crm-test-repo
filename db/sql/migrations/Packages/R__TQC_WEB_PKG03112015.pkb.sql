--
-- TQC_WEB_PKG03112015  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_web_pkg03112015 AS
...
PROCEDURE resetAgentsPassword (v_accc_code IN NUMBER, v_accc_pwd IN VARCHAR2, v_error_msg OUT VARCHAR2, v_old_pwd IN VARCHAR2 DEFAULT NULL)
IS
...
-----undoPassowrdReset
   ...
   END undoPassowrdReset;
    ...
    PROCEDURE resetUserPassword (v_user_code IN NUMBER) IS
   BEGIN
        UPDATE tqc_account_contacts
        SET   accc_pwd_reset = 'Y'
   ...
   EXCEPTION        
        WHEN OTHERS THEN
        RAISE_ERROR('An error occured while reseting the user password.');
   ...
   FUNCTION DefaultCountry(v_cou_type IN NUMBER)  RETURN Default_Country_ref
    ...
    FUNCTION recoverAgentPassword (v_entered_user IN VARCHAR2,
                              v_reset_code    IN VARCHAR2) RETURN VARCHAR2 IS
         v_usercode NUMBER;
         v_emailAddress VARCHAR2(100);

   BEGIN
        SELECT ACCC_CODE, ACCC_EMAIL_ADDR 
        INTO    v_usercode,v_emailAddress
        FROM TQC_ACCOUNT_CONTACTS
        WHERE ACCC_USERNAME = v_entered_user;
        ...
        UPDATE TQC_ACCOUNT_CONTACTS 
        SET     ACCC_PWD = gis_read.val(v_reset_code)
        WHERE ACCC_CODE = v_usercode;

        RETURN v_emailAddress;
   EXCEPTION
        WHEN NO_DATA_FOUND THEN
        RAISE_ERROR('Invalid username . Enter valid username and try again.');
   END recoverAgentPassword;
   
   PROCEDURE edit_client_details(v_clnt_code        tqc_clients.clnt_code%type,
                                                              v_clnt_name         tqc_clients.clnt_name%type,
                                                              v_clnt_passport_no         tqc_clients.clnt_passport_no%type,
                                                              v_clnt_cntct_name         tqc_clients.clnt_cntct_name_1%type,
                                                              v_clnt_cntct_phone         tqc_clients.clnt_cntct_phone_1%type,
                                                              v_clnt_email_addrs         tqc_clients.clnt_email_addrs%type,
                                                              v_clnt_tel         tqc_clients.clnt_tel%type
  )
   IS
   BEGIN
        UPDATE tqc_clients set
            clnt_name = v_clnt_name,
            clnt_tel = v_clnt_tel,
            clnt_passport_no = v_clnt_passport_no,
            clnt_email_addrs = v_clnt_email_addrs,
            clnt_cntct_name_1 = v_clnt_cntct_name,
            clnt_cntct_phone_1 = v_clnt_cntct_phone
            WHERE  clnt_code = v_clnt_code;
    EXCEPTION
        WHEN OTHERS THEN
        RAISE_ERROR('An error occurred while updating the client details');
   END;
   
   

END tqc_web_pkg03112015;
/