...
--
-- TQC_SETUPS_CURSOR140815  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_setups_cursor140815
AS
   /******************************************************************************
   ...
   BEGIN
      OPEN v_organizations_ref FOR
         SELECT org_code, org_sht_desc, org_name, org_addrs, org_twn_code,
                ...
                cou_admin_reg_type, org_mobile1, org_mobile2, cou_zip_code
...
--24/02/2015
   ...
   BEGIN
      OPEN v_countries_ref FOR
         SELECT   NULL, NULL, NULL cou_name, NULL, NULL, NULL, NULL, NULL,
...
--   PROCEDURE get_currency_rates_by_currency120815 (
--      v_crt_cur_code               tqc_currency_rates.crt_cur_code%TYPE,
--      v_currency_rates_ref   OUT   currency_rates_by_curr_ref
--   )
--   IS
...
--   END get_currency_rates_by_currency;
   ...
   --RAISE_ERROR('v_spr_spt_code '||v_spr_spt_code);
      OPEN v_service_providers_ref FOR
         SELECT   spr_code, spr_sht_desc, spr_name, spr_physical_address,
                  ...
                  spr_contact_email, spr_contact_tel,spr_tel_pay,cou_zip_code
             FROM tqc_service_providers,
                  tqc_bank_branches,
                  tqc_banks,
                  tqc_clients,
                  tqc_business_persons,tqc_countries
 ...
 FUNCTION getdirectdebitnonreceipt
   ...
   BEGIN
      ...
      SELECT dd_code,
             ...
             (SELECT bnk_bank_name
                FROM tqc_banks
               WHERE bnk_code = dd_bnk_code) bank_branch
   ...
   FUNCTION findbranches (v_reg_code NUMBER)
   ...
   BEGIN
      OPEN v_cursor FOR
         SELECT brn_code, brn_sht_desc, brn_name
           FROM tqc_branches;              -- where BRN_REG_CODE=v_reg_code ;
...
FUNCTION getgroupsmembers(v_grp_usr_code tqc_group_users.gusr_grp_usr_code%TYPE)
  ...
  BEGIN
    ---  raise_error('v_grp_usr_code---->'|| v_grp_usr_code);
    OPEN v_cursor FOR
      SELECT tqc_users.usr_code, tqc_users.usr_username, tqc_users.usr_name, gusr_code, gusr_grp_usr_code, Group_.usr_username
        FROM tqc_users, tqc_group_users, tqc_users Group_
       WHERE tqc_users.usr_code = gusr_usr_code
       AND Group_.usr_code = gusr_grp_usr_code
         AND gusr_grp_usr_code = v_grp_usr_code;
...
--RAISE_ERROR('v_rpt_rsm_code'||v_rpt_rsm_code);
   ...
   BEGIN
      OPEN v_sectors_ref FOR
         SELECT   sectors.*
             FROM (SELECT NULL sec_code, NULL sec_sht_desc, NULL sec_name,
                          NULL occ_name, NULL occ_code
                     FROM DUAL
                   UNION
...
-- RAISE_ERROR('v_bpn_payee_type'||v_bpn_payee_type||'v_bpn_clnt_code'||v_bpn_clnt_code);
   ...
   BEGIN
      OPEN v_cursor FOR
         SELECT DISTINCT agn_agent_type
                    FROM tqc_agencies
...
--   END getdistricts;
  ...
  v_county_code VARCHAR2 default null) IS
  BEGIN
