...
--
-- TQC_CLIENTS_PKG_10072016  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_clients_pkg_10072016
AS
/******************************************************************************
...
--       RAISE_ERROR('v_clnt_code '||v_clnt_code);
   ...
   BEGIN
      ...
      var_str :=
            ...
            CLNT_MARITAL_STATUS,CLNT_BPN_CODE,BPN_NAME,
            CLNT_CNTCT_RELATIONSHIP,
            CLNT_GUARDN_NAME,
            CLNT_GUARDN_AGE,
            CLNT_GUARDN_RELATIONSHIP
      FROM TQC_CLIENTS,tqc_client_accounts,tqc_countries,LMS_POLICIES,LMS_PROPOSERS,TQC_LOCATIONS,TQC_BUSINESS_PERSONS
        '
         ...
         SELECT    DISTINCT  TQC_CLIENTS.CLNT_CODE, TQC_CLIENTS.CLNT_SHT_DESC,TQC_CLIENTS.CLNT_NAME, TQC_CLIENTS.CLNT_OTHER_NAMES, TQC_CLIENTS.CLNT_ID_REG_NO, TQC_CLIENTS.CLNT_DOB, 
            ...
            CLNT_MARITAL_STATUS,CLNT_DEFAULT_COMM_MODE,CLNT_PAYROLL_NO,CLNT_SAL_MIN_RANGE,CLNT_SAL_MAX_RANGE,CLNT_BPN_CODE,BPN_NAME,CLNT_TEL_PAY,CLNT_MARITAL_STATUS,CLNT_BPN_CODE,BPN_NAME,
            CLNT_CNTCT_RELATIONSHIP,CLNT_GUARDN_NAME,CLNT_GUARDN_AGE,CLNT_GUARDN_RELATIONSHIP   
...
--        ;
      ...
      THEN
         ...
         BEGIN
            ...
            INSERT INTO tqc_clients
                 ...
                 VALUES (vclntcode, UPPER(v_pin), v_clnt_sht_desc,
 ...
 FUNCTION client_extended_proc (
      ...
      V_INT_EMAIL                 IN        VARCHAR2 DEFAULT NULL,
      v_compliance                IN        VARCHAR2 DEFAULT NULL
...
--                END;
      ...
      END IF;
   END;
-- FUNCTION client_extended_proc140815 (
--      v_direct_clnt               IN       VARCHAR2,
--      v_sht_desc                  IN       VARCHAR2,
--      v_first_name                IN       VARCHAR2,
--      v_middle_name               IN       VARCHAR2,
...
--   FUNCTION client_extended_proc120815 (
--      v_direct_clnt               IN       VARCHAR2,
--      v_sht_desc                  IN       VARCHAR2,
--      v_first_name                IN       VARCHAR2,
--      v_middle_name               IN       VARCHAR2,
...
BEGIN
   ...
   OPEN v_cursor FOR
      ...
      ORDER BY 2;
END;*/
   PROCEDURE get_tqc_accounts_agents(v_acc_type IN NUMBER,v_cursor OUT tqc_accounts_agents_cursor,v_agnt_id VARCHAR2,v_agent VARCHAR2)
    IS
    BEGIN
    --- raise_error('innnnnn '||v_acc_type);
        OPEN v_cursor FOR
        ...
        Gin_Interfaces_Pkg.bnk_account(agn_code,act_code) bnkaccount,AGN_PAYMENT_FREQ
        FROM tqc_agencies,tqc_account_types 
        WHERE agn_code != 0
        AND agn_act_code=act_code
        AND agn_act_code = v_acc_type
   ...
   BEGIN
      OPEN v_retcur FOR
         SELECT clnt_code, clnt_sht_desc, clnt_name, clnt_other_names,
                ...
                cou_admin_reg_type, prp_correspondence,clnt_cntct_relationship,
            clnt_guardn_name,clnt_guardn_age,clnt_guardn_relationship
           FROM tqc_clients,
                tqc_towns,
                tqc_countries,
                lms_proposers,
   ...
   BEGIN
      ...
      v_client_code := client_extended_proc
               ('Y',
   ...
   FUNCTION create_client_proc (
      ...
      v_prp_correspondence          lms_proposers.prp_correspondence%TYPE
            DEFAULT NULL,
                 v_clnt_cntct_relationship     IN   VARCHAR2 DEFAULT NULL, 
        v_clnt_guardn_name     IN   VARCHAR2 DEFAULT NULL, 
        v_clnt_guardn_age     IN   VARCHAR2 DEFAULT NULL, 
        v_clnt_guardn_relationship     IN   VARCHAR2 DEFAULT NULL
   --v_acc_mngr_code       IN NUMBER
   )
      RETURN NUMBER
   IS
            ...
            THEN                    --CLIENT DOESN'T EXIST, ADD THE NEW CLIENT
               INSERT INTO tqc_clients
                           (clnt_code, clnt_pin, clnt_sht_desc,
                            ...
                            clnt_marital_status,clnt_cntct_relationship, clnt_guardn_name, 
                            clnt_guardn_age, clnt_guardn_relationship
                           )
                    VALUES (vclntcode, UPPER(v_pin), v_clnt_sht_desc,
                            v_postal_addrs, v_physical_addrs,
                            ...
                            v_clnt_cntct_relationship, 
                            v_clnt_guardn_name,
                            v_clnt_guardn_age,
                            v_clnt_guardn_relationship
                           );
            ELSE
               SELECT clnt_code
                 INTO vclntcode
      ...
      THEN                                                -- This is an UPDATE
         ...
         UPDATE tqc_clients
            SET clnt_pin = UPPER(v_pin),
                ...
                clnt_cntct_relationship =  v_clnt_cntct_relationship, 
                clnt_guardn_name = v_clnt_guardn_name,
                clnt_guardn_age = v_clnt_guardn_age,
                 clnt_guardn_relationship = v_clnt_guardn_relationship
          WHERE clnt_code = v_clnt_code;
...
--END;
   ...
   BEGIN
      OPEN v_cursor FOR
         SELECT CLNTY_CLNT_TYPE,CLNTY_TYPE
          FROM TQC_CLIENT_TYPES;
   END;
   --*/ 

   --updates done on 12/01/2016
   PROCEDURE get_clnt_types(v_cursor OUT getCltTypes_ref)
    IS
    BEGIN
      OPEN V_CURSOR FOR
       SELECT CLNTY_CLNT_TYPE,CLNTY_CLNT_TYPE||'*'||DECODE(CLNTY_CATEGORY,'Corporate','C','Individual','I')
       FROM TQC_CLIENT_TYPES;
    END;
   ...
   BEGIN
      ...
      THEN
         ...
         OPEN v_retcur FOR
            ...
            CLNT_SPCL_TERMS, CLNT_DECLINED_PROP, CLNT_INCREASED_PREMIUM, CLNT_POLICY_CANCELLED, 
            CLNT_PROPOSER, CLNT_ACCNT_NO, CLNT_DOMICILE_COUNTRIES, CLNT_WEF, CLNT_WET, 
            CLNT_WITHDRAWAL_REASON, CLNT_SEC_CODE, CLNT_SURNAME, DECODE(CLNT_TYPE,'I','Individual','Corporate') CLNT_TYPE, CLNT_TITLE, CLNT_BUSINESS, 
            CLNT_ZIP_CODE, CLNT_BBR_CODE, CLNT_BANK_ACC_NO, CLNT_CLNT_CODE, 
            CLNT_NON_DIRECT, CLNT_CREATED_BY, CLNT_SMS_TEL, CLNT_AGNT_STATUS, CLNT_DATE_CREATED, 
            CLNT_RUNOFF, CLNT_LOADED_BY, CLNT_DIRECT_CLIENT, CLNT_OLD_ACCNT_NO, CLNT_USR_CODE, 
            TQC_INTERFACES_PKG.USERNAME(CLNT_USR_CODE) USR_NAME,
            ...
            CLNT_CREDIT_LIM_ALLOWED, CLNT_CREDIT_LIMIT,AGN_NAME,CLNT_DRV_EXPERIENCE,CLNT_OLD_SHT_DESC,USR_USERNAME,CLNT_BANK_PHONE_NO,
            CLNT_BANK_CELL_NO,CLT_CELL_NO,cou_name COU_DOM_CODE,clnt_occupation, clnt_occ_code
        FROM TQC_CLIENTS,TQC_AGENCY_CLIENTS,TQC_AGENCIES,TQC_USERS,TQC_COUNTRIES B
        WHERE CLNT_CODE = AGNC_CLNT_CODE(+)
         AND AGNC_AGN_CODE=AGN_CODE(+)
         AND USR_CODE(+)=CLNT_USR_CODE
         AND CLNT_DOMICILE_COUNTRIES = B.cou_code(+)
         AND CLNT_CODE=v_clnt_code;
      ELSE
         OPEN v_retcur FOR
            SELECT NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
           NULL,NULL,NULL,NULL,NULL,NULL,'A',NULL,NULL,NULL,
...
--16/02/2015 old up here
   ...
   END;
   FUNCTION get_client_dd_details(v_clnt_code IN NUMBER) RETURN client_dd_ref IS
        v_retcur client_dd_ref;
    BEGIN
   -- raise_error(v_clnt_code);
        OPEN v_retcur FOR
        SELECT clnt_code, clnt_sht_desc, clnt_name||' '||clnt_other_names, clnt_other_names, 
               clnt_id_reg_no, clnt_pin, clnt_dd_account_name, 
               clnt_dd_account_no, clnt_dd_bbr_code,bbr_branch_name
        FROM tqc_clients,tqc_bank_branches
        WHERE CLNT_CODE=v_clnt_code
        AND clnt_dd_bbr_code = bbr_code(+);
        RETURN(v_retcur);
    END;
	
procedure update_client_dd_details(v_clnt_code NUMBER,v_dd_acc_name VARCHAR2,v_dd_acc_no VARCHAR2,
   v_bbr_code NUMBER)
   IS
   BEGIN
      ...
      UPDATE TQC_CLIENTS
      SET clnt_dd_account_name =v_dd_acc_name, 
      clnt_dd_account_no =v_dd_acc_no,
       clnt_dd_bbr_code = v_bbr_code
      WHERE clnt_code =v_clnt_code;
