--
-- TQC_INTERFACES_PKG010416  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_interfaces_pkg010416
AS
...
-------------------------GET_SYS_NAME-------------------------------------------
   ...
   BEGIN
    -- RAISE_ERROR('v_process '||v_process||' v_prcs_area '||v_prcs_area||' v_prsc_sub_area '||v_prsc_sub_area||';'||v_amount||';'||v_sys_code||';'||v_usr_name||';'||v_dc);
...
--  raise_error('v_user_code=='||v_user_code||'v_sys_code=='||v_sys_code||' v_process '||v_process||' v_prcs_area '||v_prcs_area
--  ||' v_prsc_sub_area '||v_prsc_sub_area||' v_amount '||v_amount||' v_dc '||v_dc);
--  end if;   
   IF (   (v_user_code IS NULL)
          OR (v_sys_code IS NULL)
   ...
   */
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            tqc_email_pkg010416.create_email_msg
