--
-- TQC_MEMOS_DBPKG  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_memos_dbpkg
AS
   /******************************************************************************
      NAME:       tqc_memos_pkg
      PURPOSE:
...
FUNCTION process_gis_pol_memo (
      ...
      v_copdt_code     IN   NUMBER DEFAULT NULL,
   ...
   IS
      ...
      v_bbr_branch_name       VARCHAR2 (100);
      v_cmb_ipu_property_id   VARCHAR2 (50);
      v_cmb_prp_sht_desc      VARCHAR2 (50);
      v_excess_amount         VARCHAR2 (50);
      v_cpv_amount            VARCHAR2 (50);
      v_cpv_payment_mode      VARCHAR2 (50);
      v_spr_bank_acc_no       VARCHAR2 (50);
      v_csd_max_bid           NUMBER;
...
--          NULL;
      ...
      ELSIF v_app_lvl = 'C'
      THEN
--        raise_error('v_claim_no: '||v_claim_no);
         BEGIN
            SELECT DISTINCT pol_policy_no, cmb_claim_no,
                            ...
                            bbr_branch_name, cmb_ipu_property_id,
                            cmb_prp_sht_desc, clmr_amount,
                            cpv_amount, cpv_payment_mode,
                            spr_bank_acc_no, csd_max_bid
                       INTO v_pol_no, v_clm_no,
                            ...
                            v_bbr_branch_name, v_cmb_ipu_property_id,
                            v_cmb_prp_sht_desc, v_excess_amount,
                            v_cpv_amount, v_cpv_payment_mode,
                            v_spr_bank_acc_no, v_csd_max_bid
                       FROM gin_claim_master_bookings,
                            ...
                            gin_claim_recoveries,
                            gin_clm_salvages,
                            tq_crm.tqc_service_providers,
                            tq_crm.tqc_bank_branches
                      WHERE cmb_pol_batch_no = pol_batch_no(+)
                        AND cmb_claim_no = reg_cmb_claim_no(+)
                        AND ipu_pol_batch_no = pol_batch_no
                        AND cmb_ipu_code = ipu_code(+)
                        AND cmb_prp_code = clnt_code(+)
                        AND cmb_prp_code = cld_code(+)
                        AND cpv_apco_cor_code = spr_code(+)
                        AND cmb_ipu_code = ipu_code(+)
                        AND clnt_bbr_code = bbr_code(+)
                        AND cmb_claim_no = co_cmb_claim_no(+)
                        AND cmb_claim_no = clmr_cmb_claim_no(+)
                        AND cmb_claim_no = cpv_cmb_claim_no(+)
                        AND cmb_claim_no = csd_cmb_claim_no(+)
                        AND (   (cpv_ggt_trans_no = v_trans_no)
                             OR v_trans_no IS NULL
                            )
...
--           raise_error('v_trans_no'||v_trans_no);
            BEGIN
               SELECT cpv_reg_clmt_code, cpv_apco_cor_code,
                      cpv_claimant_type, cpv_net_paid, cur_symbol,
                      cpv_payment_mode, cpv_voucher_no
                 INTO v_clmt_code, v_apco_code,
                      v_clmt_type, v_cpv_amount, v_cur_symbol,
                      v_paymentmode, v_cpv_code
            ...
            EXCEPTION
               ...
               THEN
                  NULL;
--           raise_error('Transaction not found');
            END;
           IF V_PAYMENTMODE IN ('EFT','RTGS')   THEN   
            BEGIN
               ...
               LOOP
                  ...
                  FETCH bank_cursor
                   INTO v_acc_no, v_bank_name, v_branch_name,
                        v_preffered_mode;
               END LOOP;
            EXCEPTION
               WHEN OTHERS
           ...
           END IF;  
            ...
            lv_temp_txt :=
                         REPLACE (lv_temp_txt, '[CLMREVIEWDT]', v_clmreviewdt);
            lv_temp_txt :=
                    REPLACE (lv_temp_txt, '[CLMREVIEWUSER] ', v_clmreviewuser);
            lv_temp_txt :=
                     REPLACE (lv_temp_txt, '[CLMSALVAGEMAXBID]', vcsd_max_bid);
            lv_temp_txt :=
                     REPLACE (lv_temp_txt, '[CLMSALVAGEMINBID]', vcsd_min_bid);
            lv_temp_txt :=
                      REPLACE (lv_temp_txt, '[ADVOCATE]', v_advocate_handling);
            lv_temp_txt :=
                     REPLACE (lv_temp_txt, '[SALVAPPVALUE]', v_approved_value);
            ...
            lv_temp_txt :=
                      REPLACE (lv_temp_txt, '[BANKBRANCH]', v_bbr_branch_name);
            lv_temp_txt :=
                       REPLACE (lv_temp_txt, '[REGNO]', v_cmb_ipu_property_id);
            lv_temp_txt :=
                      REPLACE (lv_temp_txt, '[RISKID]', v_cmb_ipu_property_id);
            lv_temp_txt :=
                        REPLACE (lv_temp_txt, '[INSURED]', v_cmb_prp_sht_desc);
            lv_temp_txt :=
                      REPLACE (lv_temp_txt, '[EXCESSAMOUNT]', v_excess_amount);
            lv_temp_txt := REPLACE (lv_temp_txt, '[AMOUNT]', v_cpv_amount);
            lv_temp_txt :=
                       REPLACE (lv_temp_txt, '[ACCOUNTNO]', v_spr_bank_acc_no);
            lv_temp_txt := REPLACE (lv_temp_txt, '[AMOUNT]', v_amount);
            lv_temp_txt :=
                       REPLACE (lv_temp_txt, '[CURRENCYSYMBOL]', v_cur_symbol);
            lv_temp_txt :=
                         REPLACE (lv_temp_txt, '[PAYMENTMODE]', v_paymentmode);
--         EXCEPTION
--            WHEN NO_DATA_FOUND
--            THEN
--               NULL;
