--
-- TQC_WEB_PKGBKP  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.tqc_web_pkgbkp
AS
   ...
   BEGIN
      --MSG_BOX('DANI');
      BEGIN
         SELECT accc_code, TO_NUMBER (accc_code), UPPER (accc_username),
                UPPER (accc_status), accc_pwd, accc_pwd_changed,
                accc_dt_created, accc_personel_rank, accc_pwd_reset,
                accc_login_attempts,
                NVL (accc_last_login_date, TRUNC (SYSDATE)), agn_code,
                agn_name, agn_sht_desc, accc_code, accc_name,
                accc_status, agn_brn_code
           INTO nusercode, v_user_code, v_us_id,
                v_status, v_pwd, v_pwd_changed_dt,
                v_pwd_date_created, v_per_rnk, v_pwdreset,
                v_loginattempts,
                v_last_login_date, agentcode,
                agentname, agentshtdesc, agentcontactcode, agentcontactname,
                v_status, agentbrncode
...
--    MSG_BOX('DANI4');
         ...
         ELSE
            ...
            BEGIN
               INSERT INTO tqc_acc_contacts_pwd_hist
                           (accph_code, accph_accc_code,
                            accph_accc_username, accph_pwd,
                            accph_pwd_change_dt
                           )
                    VALUES (tqc_accph_code_seq.NEXTVAL, nusercode,
   ...
   BEGIN
      BEGIN
         SELECT usr_code, UPPER (usr_username), UPPER (usr_status), usr_pwd,
      ...
      THEN
         v_msg :=
            ...
            || TO_CHAR (  TO_NUMBER (v_syspwddrtntime)
...
--    MSG_BOX('DANI4');
   ...
   BEGIN
      OPEN vcursor FOR
         SELECT usar_sar_code, saa_sht_desc, saa_name, saa_sys_code,
                sys_name
   ...
   BEGIN
      ...
      BEGIN
         SELECT acwa_code, UPPER (acwa_username), UPPER (acwa_status),
                ...
                NVL (acwa_last_login_date, TRUNC (SYSDATE)), clnt_code,
                LTRIM (RTRIM (clnt_surname || ' ' || clnt_other_names)),
                acwa_code, acwa_reset_code, prp_code, prp_type,
                clnt_brn_code
           INTO nusercode, v_us_id, v_status,
                v_pwd, v_pwd_changed_dt, v_pwd_date_created,
                v_per_rnk, v_pwdreset, v_loginattempts,
                ...
                v_defaultbranch
           FROM tqc_client_web_accounts, tqc_clients, lms_proposers
          WHERE clnt_code(+) = acwa_clnt_code
   ...
   BEGIN
      ...
      OPEN v_refcur FOR
         SELECT DISTINCT brn_code, brn_sht_desc,
                         org_sht_desc || '-' || brn_name, brn_gen_pol_clm
                    FROM tqc_branches,
                         tqc_regions,
                         tqc_organizations,
                         tqc_systems
      ...
      ELSE
         BEGIN
            INSERT INTO tqc_bpm_tickets
                        (tckt_code, tckt_sys_code, tckt_sys_module,
                         tckt_clnt_code, tckt_agn_code, tckt_pol_code,
                         tckt_pol_no, tckt_clm_no, tckt_quot_code, tckt_by,
                         tckt_date, tckt_process_id, tckt_quo_no,
                         tckt_endr_code, tckt_prod_type, tckt_to,
                         tckt_remarks, tckt_endorsement, tckt_transno,
                         tckt_type, tckt_extern_ref_no, tckt_adhoc_name
                        )
                 VALUES (v_tckt_code, v_sys_code, v_sys_module,
                         v_clnt_code, v_agn_code, v_pol_code,
                         v_pol_no, v_clm_no, v_quot_code, v_user,
                         SYSDATE, v_process_id, v_quot_no,
                         v_endr_code, v_prod_type, v_tckt_to,
                         v_remarks, v_endorsment, v_trans_no,
                         v_type, v_extern_ref_no, v_adhoc_name
                        );
   ...
   BEGIN
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error (   'error determining the task point...'
                         || v_task_point
                         || '='
                         || v_taskid
                        );
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error (   'error determining the task details...'
                         || v_taskid
                         || '='
                         || v_task_point
                        );
      ...
      OPEN v_refcur FOR
         ...
         */
         SELECT   tqc_bpm_tickets.tckt_code,
                  ...
                  tqc_bpm_tickets.tckt_agn_code, agn_name AGENT,
                  ...
                  NULL, NULL, tckt_endorsement, tckt_transno,
   ...
   BEGIN
      OPEN v_refcur FOR
         SELECT   tqc_bpm_tickets.tckt_code,
                  ...
                  fms_cheques.cqr_no, fms_cheques.cqr_payee,
                  fms_cheques.cqr_narrative, tqc_bpm_tickets.tckt_by,
                  tqc_bpm_tickets.tckt_date, tqc_bpm_tickets.tckt_process_id,
                  tqc_bpm_tickets.tckt_sys_module, tqc_bpm_tickets.tckt_to,
                  tqc_bpm_tickets.tckt_remarks, name_ tckt_name,
   ...
   BEGIN
      ...
      */
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            raise_error ('error parsing Calendar dates...'
                         || SQLERRM (SQLCODE)
                        );
      END;
      ...
      INSERT INTO tqc_calendar_activities
                  (cala_code, cala_location, cala_title,
                   cala_startadddays, cala_starthour, cala_endadddays,
                   cala_endaddhours, cala_user, cala_create_date
                  )
           VALUES (calendar_activities_seq.NEXTVAL, v_loc, v_title,
                   v_st_days, v_st_hrs, v_end_days,
                   v_end_hrs, v_user, v_create_date
                  );
   EXCEPTION
      WHEN OTHERS
      THEN
...
--       RAISE_ERROR(' v_batch_no == '||v_batch_no);
         ...
         BEGIN
            INSERT INTO tqc_bpm_tickets
                        (tckt_code, tckt_sys_code, tckt_sys_module,
                         ...
                         tckt_endr_code, tckt_prod_type, tckt_to,
                         tckt_remarks, tckt_endorsement, tckt_transno,
                         tckt_prp_code, tckt_cla_type, tckt_cqr_no
                        )
                 VALUES (v_tckt_code, v_sys_code, v_sys_module,
                         v_clnt_code, v_agn_code, v_pol_code,
                         v_pol_no, v_clm_no, v_quot_code, v_user,
                         ...
                         v_endr_code, v_prod_type, v_tckt_to,
                         v_remarks, v_endorsment, v_trans_no,
                         v_prp_code, v_cla_type, v_tckt_cqr_no
                        );
         --COMMIT;
         EXCEPTION
            WHEN OTHERS
   ...
   BEGIN
      ...
      THEN
         BEGIN
            ...
            SELECT tqc_rdoc_code_seq.NEXTVAL
              INTO v_new_rdoc_code
              FROM DUAL;

            INSERT INTO tqc_required_documents
                        (rdoc_code, rdoc_sht_desc, roc_desc,
                         roc_mandatory, roc_date_added
                        )
                 VALUES (v_new_rdoc_code, v_rdoc_sht_desc, v_rdoc_desc,
                         v_rdoc_mandatory, NVL (v_rdoc_date, SYSDATE)
                        );
         EXCEPTION
            WHEN OTHERS
            THEN
   ...
   BEGIN
      ...
      THEN
         BEGIN
            SELECT tqc_cdocr_code_seq.NEXTVAL
              INTO v_new_cdocr_code
              FROM DUAL;

            INSERT INTO tqc_client_documents
                        (cdocr_code, cdocr_rdoc_code, cdocr_clnt_code,
                         cdocr_submited, cdocr_date_s,
                         cdocr_ref_no, cdocr_rmrk, cdocr_user_receivd
                        )
                 VALUES (v_new_cdocr_code, v_rdoc_code, v_clnt_code,
                         v_submitted, NVL (v_date_submitted, SYSDATE),
                         v_ref_no, v_rmrk, v_user
                        );
         EXCEPTION
            WHEN OTHERS
            THEN
   ...
   BEGIN
      ...
      LOOP
         INSERT INTO tqc_client_documents
                     (cdocr_code, cdocr_rdoc_code, cdocr_clnt_code,
                      cdocr_submited, cdocr_date_s, cdocr_user_receivd
                     )
              VALUES (tqc_cdocr_code_seq.NEXTVAL, rd.rdoc_code, v_clnt_code,
                      'N', NULL, v_user
                     );
      END LOOP;
   END;
      ...
      THEN
         BEGIN
            INSERT INTO tqc_memo_types
                        (mtyp_code, mtyp_sys_code, mtyp_memo_type,
                         mtyp_saa_code, mtyp_status, mtyp_sub_code,
                         mtyp_appl_lvl
                        )
                 VALUES (tqc_mtyp_code_seq.NEXTVAL, v_sys_code, v_memo_type,
                         v_sy_app_code, v_status, v_sub_class,
                         v_appllvl
                        );
         END;
      ELSIF v_action = 'E'
   ...
   BEGIN
      ...
      THEN
         BEGIN
            SELECT tqc_memo_code_seq.NEXTVAL
              INTO v_new_memo_code
              FROM DUAL;
            ...
            INSERT INTO tqc_memo_details
                        (memdet_code, memdet_content,
                         memdet_memo_code
                        )
                 VALUES (tqc_memdet_code_seq.NEXTVAL, v_content,
                         v_new_memo_code
                        );
         EXCEPTION
            WHEN OTHERS
   ...
   IS
      CURSOR tickets (vpolcode IN NUMBER)
      IS
   ...
   BEGIN
      OPEN v_tickets FOR
         SELECT pol_batch_no, pol_policy_no,
                clnt_other_names || ' ' || clnt_name AS client,
                pol_policy_status,
                DECODE (NVL (pol_authosrised, 'N'),
                        'N', 'No',
                        'Yes'
                       ) authorised,
                clnt_code
   ...
   BEGIN
      OPEN v_tickets FOR
         SELECT quot_code, quot_no,
                clnt_other_names || ' ' || clnt_name client, quot_status,
                DECODE (NVL (quot_confirmed, 'N'),
                        'N', 'No',
                        'Yes'
                       ) confirmed,
                clnt_code
         ...
         SELECT   *
             FROM (SELECT a.*
                   ...
                   SELECT tqc_bpm_tickets.tckt_code,
                          ...
                          tqc_bpm_tickets.tckt_quot_code,
                          tqc_bpm_tickets.tckt_quo_no,
                          tqc_bpm_tickets.tckt_by, tqc_bpm_tickets.tckt_date,
                          tqc_bpm_tickets.tckt_process_id,
                          tqc_bpm_tickets.tckt_sys_module sys_module_code,
                          tckt_endr_code, tckt_prod_type, tckt_to,
                          tckt_remarks, name_ tckt_name,
                          duedate_ tckt_due_date, tckt_endorsement,
                          tckt_transno, tckt_extern_ref_no ref_no,
                          tckt_prp_code, NULL, tckt_type
                     FROM tqc_bpm_tickets,
                          tqc_clients,
                          tqc_agencies,
             ...
             FROM (SELECT a.*
                   ...
                   SELECT tqc_bpm_tickets.tckt_code,
                    ...
                    WHERE tqc_bpm_tickets.tckt_clnt_code = clnt_code(+)
                      ...
                      AND agn_code(+) = tqc_bpm_tickets.tckt_agn_code
                      AND dbid_ = tqc_bpm_tickets.tckt_code
                      AND tckt_sys_module IN ('P', 'U', 'R', 'RT', 'RE')
                      --AND TCKT_CLM_NO = NVL(:v_claim_no, TCKT_CLM_NO)
                      AND tckt_active != 'N')
         ORDER BY tckt_code DESC;
   ...
   BEGIN
      ...
      BEGIN
         SELECT accc_code, TO_NUMBER (accc_code), UPPER (accc_username),
                ...
                accc_dt_created, accc_personel_rank, accc_pwd_reset,
                accc_login_attempts,
                NVL (accc_last_login_date, TRUNC (SYSDATE)), cou_code,
                cou_name, NULL agn_sht_desc, accc_code, accc_name,
                accc_status, NULL agn_brn_code
           INTO nusercode, v_user_code, v_us_id,
                v_status, v_pwd, v_pwd_changed_dt,
                v_pwd_date_created, v_per_rnk, v_pwdreset,
                v_loginattempts,
                v_last_login_date, coucode,
                couname, agentshtdesc, agentcontactcode, agentcontactname,
                v_status, agentbrncode
   ...
   BEGIN
      ...
      UPDATE tqc_users
         SET usr_pwd_reset_code = v_reset_code,
             usr_pwd_reset = 'Y'
   ...
   BEGIN
      ...
      UPDATE tqc_client_web_accounts
         SET acwa_reset_code = v_reset_code,
             acwa_pwd_reset = 'Y'
...
--RAISE_ERROR('v_usr_security_question'||v_usr_security_question||'v_usr_security_answer'||v_usr_security_answer||'v_usr_username'||v_usr_username);
   ...
   PROCEDURE user_log_out (v_user NUMBER, v_sys_code NUMBER, v_type VARCHAR2)
   IS
      vdbuser      VARCHAR2 (20);
   ...
   BEGIN
      ...
      THEN
         UPDATE tqc_user_logon
            SET usl_logout_dt = SYSDATE,
                usl_log_out_type = v_type
          WHERE TO_CHAR (usl_session_code) NOT IN (
                      SELECT SUBSTR (module,
                                     INSTR (module, ':') + 1
                                    ) killed_sid
                        FROM tqc_session_log)
            AND usl_log_out_type IS NULL
            AND TRUNC (usl_logon_dt) = TRUNC (SYSDATE)
            AND usl_sys_code = v_sys_code;

         RETURN;
      END IF;

      BEGIN
         SELECT sys_dbase_username, UPPER (usr_username)
           INTO vdbuser, v_username
           FROM tqc_user_systems, tqc_users, tqc_systems
          WHERE usys_usr_code = usr_code
            AND usr_code = v_user
      ...
      FOR cur_session_rec IN cur_session (v_username, vdbuser)
      LOOP
         --RAISE_ERROR('cur_session_rec.killed_sid '||cur_session_rec.killed_sid);
         UPDATE tqc_user_logon
            SET usl_logout_dt = SYSDATE,
                usl_log_out_type = v_type
          WHERE usl_usr_code = v_user
            AND usl_session_code = cur_session_rec.killed_sid;
      END LOOP;
   EXCEPTION
      WHEN OTHERS
      THEN
         raise_error ('ERROR LOGGING OUT' || SQLERRM (SQLCODE));
   END user_log_out;
      ...
      ELSE
         OPEN v_cursor FOR
            SELECT usr_email
             ...
             WHERE gusr_grp_usr_code = v_usrcode AND gusr_usr_code = usr_code;
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         raise_error ('ERROR FINDING USER EMAIL' || SQLERRM (SQLCODE));
   END;
END tqc_web_pkgbkp; 
/