...
--
-- TQC_SETUPS_CURSOR_BACK  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY TQ_CRM.TQC_SETUPS_CURSOR_back AS
  /******************************************************************************
...
-- RAISE_ERROR('v_bpn_payee_type'||v_bpn_payee_type||'v_bpn_clnt_code'||v_bpn_clnt_code);
   OPEN v_cursor FOR
      ...
      BBR_BRANCH_NAME,COU_NAME,NULL BPN_PA7YEE_TYPE
...
IS
   ...
   begin
   OPEN v_cursor FOR
     SELECT DISTINCT AGN_AGENT_TYPE FROM TQC_AGENCIES
...
IS
   ...
   begin
   OPEN v_cursor FOR
    SELECT DISTINCT AGN_GROUP FROM TQC_AGENCIES WHERE AGN_GROUP IS NOT NULL;
       RETURN (v_cursor);
END;
...
IS
   ...
   begin
   OPEN v_cursor FOR
    SELECT  OCC_CODE, OCC_SEC_CODE, OCC_SHT_DESC, OCC_NAME FROM tqc_occupations;
       RETURN (v_cursor);
END;

PROCEDURE get_postal_codes(v_locations_ref OUT locations_ref) IS
  BEGIN
    OPEN v_locations_ref FOR
      SELECT NULL loc_code, NULL loc_twn_code, pc_code, pc_name
