--
-- PROCESS_GIS_POL_MEMO_2  (Function) 
--
CREATE OR REPLACE FUNCTION TQ_CRM.process_gis_pol_memo_2 (
...
IS
   ...
   v_cpv_code         NUMBER;
   v_loss_date_time   DATE;
   v_osrsv            NUMBER;
   v_sub_class VARCHAR2 (300);
   ...
   THEN
      ...
      THEN
         BEGIN
            ...
            raise_error ('HERE');
            lv_temp_txt :=
               REPLACE (lv_temp_txt, '[WEF]',
                        TO_CHAR (veffdate, 'DD/MM/YYYY'));
            lv_temp_txt :=
               REPLACE (lv_temp_txt, '[WET]',
   ...
   THEN
      ...
      BEGIN
         SELECT bon_obligor, obligor_addrs, bon_obligor_town,
          ...
          WHERE bon_ipu_code = ipu_code AND ipu_pol_batch_no = v_pol_batch_no;
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            NULL;
         ...
         EXCEPTION
            ...
            THEN
               SELECT DISTINCT pol_policy_no, cmb_claim_no,
                               ...
                               pol_tot_endos_diff_amt, cmb_claim_date,
                               cmb_loss_date_time,
                               cmb_ipu_property_id risk_id, cmb_location,
                               cmb_loss_desc,
                               NVL (cmb_other_cover_details,
                                    'Repairable loss') loss_type,
                               ipu_item_desc risk_description,
                               DECODE (reg_third_party,
                                       'S', 'SELF',
                                       'THIRD PARTY'
...
--     RAISE_ERROR('v_claim_no'||v_claim_no||'v_gcc_code'||v_gcc_code);
...
--            raise_error('v_trans_no'||v_trans_no);
         BEGIN
            SELECT cpv_reg_clmt_code, cpv_apco_cor_code, cpv_claimant_type,
                   cpv_net_paid, cur_symbol, cpv_payment_mode, cpv_voucher_no
              INTO v_clmt_code, v_apco_code, v_clmt_type,
                   v_amount, v_cur_symbol, v_paymentmode, v_cpv_code
              FROM gin_clm_payment_vouchers, tqc_currencies
             WHERE cpv_ggt_trans_no = v_trans_no AND cur_code = cpv_cur_code;
         EXCEPTION
            WHEN OTHERS
            THEN
               NULL;
...
--raise_error ('ERROR: ' || SQLERRM (SQLCODE));
         ...
         lv_temp_txt := REPLACE (lv_temp_txt, '[LOSSDATE]', v_loss_date_time);
         lv_temp_txt := REPLACE (lv_temp_txt, '[OSRSV]', v_osrsv);
          lv_temp_txt := REPLACE (lv_temp_txt, '[SUBCLASS]', v_sub_class);
--         EXCEPTION
--            WHEN NO_DATA_FOUND
--            THEN
--               NULL;
      ...
      THEN
         BEGIN
            SELECT quot_no, quot_tot_property_val,
                   gis_utilities.clnt_name (clnt_name, clnt_other_names)
                                                                    clnt_name,
                   quot_premium, quot_cover_from, quot_cover_to, quot_date,
                   agn_name, brn_name, clnt_postal_addrs, twn_name,
                   qr_property_id, qr_item_desc, qr_wef, qr_wet,
                   qr_covt_sht_desc, qr_premium, qr_value, quot_cur_symbol
              INTO v_pol_no, v_sa,
                   v_client,
                   v_prem, vcoverfrom, vcoverto, veffdate,
                   v_agent, v_branch, v_address, v_twn,
                   v_risk_id, v_risk_desc, v_risk_wef, v_risk_wet,
                   v_cover, v_risk_prem, v_risk_value, v_currency
              FROM gin_quotations,
                   tqc_clients,
                   tqc_agencies,
                   tqc_branches,
