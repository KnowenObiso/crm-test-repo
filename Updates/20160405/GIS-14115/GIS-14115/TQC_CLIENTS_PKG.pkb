   ...
   BEGIN
    ...
    -- RAISE_ERROR(' v_firstname '||v_firstname||' v_middle_name '||v_middle_name||' v_surname '||v_surname);
      ...
      THEN
        ...
        --RAISE_ERROR('v_surnamX: '||NVL (v_firstnamei,v_surnamei));
         IF NVL (v_firstnamei,v_surnamei) = 'A'
         THEN
...
FUNCTION client_extended_proc (
      ...
      v_clnt_sms_prefix           IN       VARCHAR2 DEFAULT NULL,
...
--   raise_error(
--      'v_client_category: '||v_client_category ||
--      'v_clnt_sms_prefix: '||v_clnt_sms_prefix 
--   );
--      raise_error(
--      'v_clnt_bpn_code: '||v_clnt_bpn_code ||
--      'v_clnt_client_level: '||v_clnt_client_level||
--      'v_clnt_tel_pay: '||v_clnt_tel_pay
--      );
    --RAISE_ERROR('v_payroll '||v_payroll||' v_minSalary '||v_minSalary||' v_minSalary '||v_minSalary);
--  RAISE_ERROR('v_sec_code='||v_sec_code);
     -- raise_error('v_clnt_compliance: '||v_clnt_compliance);
     ...
     EXCEPTION
        ...
        THEN
           v_count1  := 0;
     END;
    -- RAISE_ERROR('v_count1 '||v_count1);
--        IF NVL (v_count1, 0) >= 1
--        THEN
--        raise_error ('Phone number already exists....' ||  v_decodesmstel
--                     || ' times'
--                    );
--         END IF;

      IF (v_id_reg_no IS NOT NULL) AND (NVL(v_add_edit,'*') <> 'E')
      THEN
         BEGIN
            SELECT NVL (COUNT (*), 0)
              INTO v_id_cnt
      ...
      THEN
         ...
         IF NVL (v_id_cnt, 0) >= 1
         THEN
            raise_error ('Pin number already exists....' || v_id_cnt
                         || ' times'
                        );
         END IF;
      END IF;
     ...
     IF v_sms_tel IS NOT NULL
      THEN
         ...
         FROM DUAL ;
         --RAISE_ERROR('v_sms_tel_count='||v_sms_tel_count);
         IF NVL(v_sms_tel_count,0) = 0 THEN 
             BEGIN
     ...
     -- RAISE_ERROR('v_decodesmstel: '||v_decodesmstel||'v_sms_tel='||v_sms_tel);
        BEGIN
            SELECT PARAM_VALUE
            INTO v_clnt
            FROM TQC_PARAMETERS
         ...
         THEN
            ...
            THEN
               ...
               EXCEPTION
                  WHEN NO_DATA_FOUND
                  THEN
                     raise_error ('PLEASE RESELECT BRANCH');
               END;
            END IF;
         ...
         THEN
            BEGIN
                 SELECT occ_code  
                  ...
                  WHERE  occ_name= v_clnt_occupation;
              EXCEPTION
                 WHEN OTHERS
                 THEN
...
-- END IF;
               BEGIN
                  ...
                  IF NVL(v_org_type,'*')='INS' THEN
                      create_client_account ('A',
                                             NULL,
                                             NULL,
...
-- RAISE_ERROR('You cannot Create more than two accounts in the same division '||v_user);
      ...
      THEN                                                -- This is an UPDATE
         BEGIN
           -- RAISE_ERROR('v_decodesmstel'||v_decodesmstel||'Error creating the client..' ||v_clnt_code);
            UPDATE tqc_clients
               SET clnt_pin = UPPER (v_pin),
                   ...
                   clnt_email_addrs = v_email_addrs,
                   clnt_tel = v_tel,
                   clnt_sms_tel = NVL(v_decodesmstel,clnt_sms_tel),
                   clnt_fax = v_fax,
                   clnt_sec_code = v_sec_code,
                   ...
                   clnt_comm_mode = v_clnt_bpn_code,
                   ...
                   clnt_sms_prefix = v_clnt_sms_prefix,
                   clnt_drv_dl_issue_dt = v_clnt_drv_experience,
                   clnt_locked = 'N',
                   clnt_unlocked_by = UPPER (v_user),
                   clnt_unlocked_on = SYSDATE,
                   clnt_spz_code=v_clnt_spz_code
                --clnt_dl_issue_date = v_clnt_drv_experience
             --  CLNT_CLNT_CODE=  v_clnt_clnt_code
            WHERE  clnt_code = v_clnt_code;
...
--PROCEDURE create_client_account07032017 (
--      v_add_edit       VARCHAR2,
--      v_acc_code       NUMBER,
--      v_sht_desc       VARCHAR2,
--      v_name           VARCHAR2,
...
--      v_existing_div     NUMBER;
--      v_div_sht_desc    VARCHAR2 (50);
--      v_div_brn_code        NUMBER;
--      vuser             VARCHAR2 (35)
...
--             
--             
--             SELECT BDIV_BRN_CODE INTO v_div_brn_code 
--             from tqc_brnch_divisions
--             WHERE BDIV_DIV_CODE=v_old_div
--              AND ROWNUM=1
--             ORDER BY BDIV_WEF DESC;
--            
--             
--         END;
...
--            then
--               gis_utilities.migrate_acc_brn_div(v_acc_code,v_div_brn_code,v_div_code);
--            end if;
--           
--         
--            UPDATE tqc_client_accounts
...
--*update this procedure in TQC_CLIENTS_PKG*
...
PROCEDURE create_client_account (
   ...
   BEGIN
   V_CLNT_BRN_CODE :=v_brn_code;
       BEGIN
           SELECT DISTINCT (BDIV_BRN_CODE) 
           INTO V_CLNT_BRN_CODE 
           FROM TQC_BRNCH_DIVISIONS 
           WHERE BDIV_DIV_CODE =v_div_code;
       EXCEPTION WHEN OTHERS
        THEN
            RAISE_ERROR('DIVISON DIFINED IN MULTIPLE BRANCHES');
       END;
--   RAISE_ERROR('v_acc_code: '||v_acc_code);
--      raise_error(
--      'v_clna_cnt_name: '||v_clna_cnt_name||
...
-- RAISE_ERROR('You cannot Create more than two accounts in the same division ');
      ...
      THEN
         ...
         BEGIN
            ...
            if v_old_div != v_div_code
            then
               gis_utilities.migrate_acc_brn_div(v_acc_code,V_CLNT_BRN_CODE,v_div_code);
            end if;
...
--        END IF;
   ...
   PROCEDURE get_accounts_clients (
      ...
      v_client_id         VARCHAR2 DEFAULT NULL,
      v_client            VARCHAR DEFAULT NULL
   )
   IS
   BEGIN
      OPEN v_cursor FOR
...
PROCEDURE get_tqc_accounts_agents (
      v_acc_type   IN       NUMBER,
      v_cursor     OUT      tqc_accounts_agents_cursor,
      v_agnt_id  IN       VARCHAR2 default null,
      v_agent in      varchar2 default null
   )
   IS
   BEGIN
      OPEN v_cursor FOR
   ...
   PROCEDURE get_gis_clients (
      v_cursor        OUT   tqc_accounttrans_cursor,
      v_client_id           VARCHAR2  DEFAULT NULL,
      v_client_name         VARCHAR2  DEFAULT NULL
   )
   IS
   BEGIN
      OPEN v_cursor FOR
