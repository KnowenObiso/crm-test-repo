...
public class MakerCheckerManipulation {
    ...
    private RichSelectOneChoice applicableArea;

    public MakerCheckerManipulation() {
...
(OracleCallableStatement)conn.prepareCall(grantQuery);
  ...
  public String actionAuthorizeSystemRole() { 
    ...
    public void processSelectedNew(ValueChangeEvent valueChangeEvent) {
        ...
        JUCtrlHierNodeBinding nd = (JUCtrlHierNodeBinding)rolesTree.getRowData();
        BigDecimal mkccode = GlobalCC.checkBDNullValues(nd.getRow().getAttribute("mkcCode")); 
        String mkcStatus;// =            (String)nd.getRow().getAttribute("mkcStatus");  
        String action;
    
//        applicableArea =GlobalCC.checkNullValues(applicableArea.toString() );
       session.setAttribute("applicableArea", this.applicableArea.getValue());
            String applicableArea =  GlobalCC.checkNullValues(session.getAttribute("applicableArea"));
            if(mkccode==null){

         ADFUtils.findIterator("findSystemUserRightsIterator").executeQuery();
         GlobalCC.refreshUI(rolesTree);
            }
           // System.out.println("here" + applicableArea);
        if(processCheck.isSelected()&& (applicableArea==null||applicableArea.equals("S") ))
        {
           
            action="D";
            mkcStatus="INACTIVE";
            changeRolesMakerchecker(mkccode,mkcStatus,null,action);
            ADFUtils.findIterator("findSystemUserRightsIterator").executeQuery();
            GlobalCC.refreshUI(rolesTree);
            GlobalCC.INFORMATIONREPORTING("Please select Applicable Area first");
            return;
        }
        else if(processCheck.isSelected()&& (applicableArea != null|| !applicableArea.equals("S")))
        {
            action="E";
            mkcStatus="ACTIVE";
            changeRolesMakerchecker(mkccode,mkcStatus, applicableArea,action);
          //  System.out.println("here" + applicableArea+"here2"+ action);
            ADFUtils.findIterator("findSystemUserRightsIterator").executeQuery();
            GlobalCC.refreshUI(rolesTree);
            //GlobalCC.INFORMATIONREPORTING("Please select Applicable Area first");
           // return;
        }
        else if  (processCheck.isSelected() == false && (applicableArea != null|| !applicableArea.equals("S")))
         {
             action="E";
             mkcStatus="INACTIVE";
             changeRolesMakerchecker(mkccode,mkcStatus, null,action);
  ...
  public RichInputText getTxtSysRoleAuthorized()
  {
    ...
    public void populateSysRights(ValueChangeEvent valueChangeEvent){
            try{
                BigDecimal sysCode=GlobalCC.checkBDNullValues(populatedSystems.getValue());
                session.setAttribute("sysCode",sysCode);
                session.removeAttribute("roleCode");
    ...
    public void setApplicableArea(RichSelectOneChoice applicableArea) {
        this.applicableArea = applicableArea;
    }

    public RichSelectOneChoice getApplicableArea() {
        return applicableArea;
    }
    public void changeRolesMakerchecker(BigDecimal mkcCode,String mkcStatus, String mkcApplicableArea,String action) {
        DBConnector datahandler = new DBConnector();
        OracleConnection conn;
        conn = datahandler.getDatabaseConnection();
        
        if(mkcCode==null){
            GlobalCC.INFORMATIONREPORTING("Please select Process Sub Area first");
            return;
        }

        try {
            String authorizeQuery ="begin tqc_roles_pkg.update_rolesmakercheckerstatus(?,?,?,?); end;";

            OracleCallableStatement cst = null;
            cst = (OracleCallableStatement)conn.prepareCall(authorizeQuery);
            cst.setBigDecimal(1, mkcCode);
            cst.setString(2, mkcStatus);
            cst.setString(3,mkcApplicableArea);
            cst.setString(4,action);
            cst.execute();
